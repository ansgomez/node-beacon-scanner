...
const mMiroCard = require('./parser-mirocard.js');

// DEBUGGING VARIABLES
const DEBUG = false; // true when debugging
const MIROCARD_ONLY = true; // when true, filters out non-MiroCard beacons

// MIROCARD VARIABLES
const MIROCARD_MAC = "607771";
...
BeaconParser.prototype.parse = function (peripheral) {
	...
	// //MiroCard
	if (beacon_type === 'mirocard') {
		parsed = mMiroCard.parse(peripheral);
	}
...
BeaconParser.prototype._detectBeaconType = function (peripheral) {
	...
	// MiroCard
	if (peripheral.id.startsWith(MIROCARD_MAC)) {
		return 'mirocard';
	}
	// Unknown
	return '';
